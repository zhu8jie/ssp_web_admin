      ...
      <div class="data-table console-table">
        ...
        >
          ...
          <template slot-scope="{ row }" slot="date">
            <p class="table_text"> {{ row.date | timeFormat('HH:mm:ss') }}</p>
          </template>
          <!--计划 -->
          ...
          <template slot-scope="{ row }" slot="ssp_slot_name">
			  <div class="control-flex">
				  ...
				  <a class="control-right" target="_blank" :href="'/ssp/ad/manage?sspSlotId=' + row.ssp_slot_id">
					  <Icon type="md-create"/>
				  </a>
			  </div>
          </template>
  ...
  export default {
    ...
    methods: {
      ...
      getParentColumns () {
        ...
        ColumnList.map(item => {
          if (extraMust.includes(item.slot) || this.trafficHasCheckData.indexOf(item.title) > -1) {
      ...
      async _tableInfoApi() {
        ...
        // 流量表格 - 设置  -  主要是用来判断是否全选
        if (resArr.table_head_head_16) {
          let curArr = resArr.table_head_head_16.split(',')
      ...
      submitData() {
        ...
        this.setTableHeadApi({table_id: 16, select_str: this.trafficHasCheckData.join(',')}).then(() => {
