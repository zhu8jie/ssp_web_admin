  ...
  <Card :bordered="false" dis-hover class="page-main ad_config">
    ...
    <div class="config_con">
      ...
      >
              ...
              >
                ...
                >
                  <Option
                    ...
                    :value="item.id"
                    :key="'公司' + item.id + item.company_name"
                    >{{ item.company_name }}
                  </Option>
                </Select>
              </FormItem>
        ...
        <div class="config_detail">
          <div class="config_header">
            <div><span class="line"></span><span>投放设置</span></div>
            <div>
                          ...
                          <div class="xuan_content" style="display: flex">
                            ...
                            >
                              <Option
                                v-for="item in it.dealList"
                                :value="item.id"
                                :key="item.created_at + 'deal'"
                                >{{ item.name }}</Option
                              >
                            </Select>
                            <Alert
                              v-if="it.dealList && it.dealList.length > 0"
                              class="deal_alert"
                              show-icon
              ...
              >
                ...
                >
                  ...
                  <template slot-scope="{ row }" slot="dsp_company_id">
                    <p class="name-row-ellipsis" :title="row.dsp_company_name">
                      {{ row.dsp_company_name }}
                    </p>
...
export default {
  ...
  data() {
    const validateRatio = debounce((rule, value, callback) => {
      ...
      if (
        _pre === 0 ||
        _next === 0 ||
        !regNumPositiveInteger.test(_pre) ||
        !regNumPositiveInteger.test(_next)
      ) {
        callback(new Error("请选择正确的素材尺寸比例"));
      } else {
            ...
            (res) => {
              if (res.code === 200) {
                ...
                if (this.ratioErrSlotList.length === 0) {
                  ...
                  callback(
                    new Error(
                      `存在不匹配的广告位${this.ratioErrSlotList.length}个`
                    )
                  );
                }
              } else {
            ...
            (res) => {
              if (res.code === 200) {
                ...
                if (this.osTypeErrSlotList.length === 0) {
                  ...
                  callback(
                    new Error(
                      `存在不匹配的广告位${this.osTypeErrSlotList.length}个`
                    )
                  );
                }
              } else {
    ...
    return {
      ...
      modalFormRules: {
        ...
        pay_type: [{ required: true, validator: validatePayTypeFn, trigger: "change" }],
        os_type: [
      ...
      slot_id: this.$route.query.slot_id
        ? Number(this.$route.query.slot_id)
        : "", // 预算位的ID
    };
  },
  filters: {
  ...
  computed: {
     ...
     */
    sizeProportion() {
      return number2Thousand(
        this.modalForm.dsp_ad_ratio_width / this.modalForm.dsp_ad_ratio_height
      );
    },
    /**
     * [osType 应用平台]
  ...
  methods: {
      ...
      controllerList.map((item, q) => {
        ...
        let obj1 = {
          ...
          title:
            ...
            item.ssp_slot_id +
            "，终端:" +
            item.ssp_slot.app_os_type_text, // id以及终端的文本
          os_type: type,
          ...
          control_time_custom:
            item.control_time_type === 1
              ? item.control_time_custom_bin.split("").reverse().join("")
              : "00000000000000000000000", // 投放的时段
          timeID: "timeID" + item.ssp_slot_id + q, // 投放时段的插件
          cityList: this.getCityList(), // 地域的列表
          area_codes:
            item.area_type !== -1 && item.area_code
              ? item.area_code.split(",")
              : [], // 地域选中项
          ...
          brand_ids:
            item.brand_type !== -1 && item.brand_id
              ? item.brand_id.split(",")
              : [], // 手机品牌
          // 人群定向
          defaultCrowd: {
            ...
            tag_list:
              item.tag_list && item.tag_list.length > 0 ? item.tag_list : [],
          },
    ...
    selectSuccess(selectedData) {
      ...
      let scale = selectedData.ad_ratio.map((item) => {
        return item
          ? Number(this.number2Thousand(item.width / item.height))
          : 0;
      });
      let listData = {};
      ...
      listData.title =
        "ID:" + selectedData.ssp_slot_id + "，终端:" + selectedData.app_os_type;
    ...
    errData() {
      ...
      if (errOsList.length > 0 || errAdList.length > 0) {
        Object.keys(errOsList).forEach((h) => {
          ...
          html +=
            '<p style="font-size:14px;color:#333;line-height: 25px">当前预算位所配置的广告位：【' +
            errOsList[h].ssp_slot_name +
            "】的应用平台是【" +
            type +
            "】，与当前预算位应用平台【" +
            typeVerify +
    ...
    inData() {
      ...
      this.basicDspStatus = parseInt(localStorage.getItem("basicDspStatus"))
        ? parseInt(localStorage.getItem("basicDspStatus"))
        : 1;
      this.putDSPStatus = parseInt(localStorage.getItem("putDSPStatus"))
        ? parseInt(localStorage.getItem("putDSPStatus"))
        : 1;
    },
  },
  watch: {
