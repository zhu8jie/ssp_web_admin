  ...
  <Card :bordered="false" dis-hover class="page-main">
    <div class="page-title clearfix">
      ...
      <div class="is-handler">
        <Button
          style="padding: 0 40px; color: #3f83f7"
          type="text"
          @click="getBack"
          >返回 ></Button
        >
        ...
        <Form label-position="left" :label-width="192">
          ...
          >
            ...
            >
              ...
              <div
                style="width: 200px; height: 200px; line-height: 200px"
                v-show="seen && 1 === current"
                class="account_shade"
                @click="showImg(ruleForm.business_license)"
              >
          ...
          <FormItem class="appForm" label="联系地址">
            <p class="text">
              <span
                ...
                v-if="value.id === ruleForm.province_id"
                >{{ value.name }}</span
              >
              <span
                v-for="(value, index) in cityList1"
                :key="'cityList1' + index"
                v-if="value.id === ruleForm.city_id"
                >{{ value.name }}</span
              >
          ...
          <!-- <FormItem class="appForm" label="合作主体">
            <p class="text" v-if="ruleForm.cooperation_company">
              {{ ruleForm.cooperation_company === 1 ? "聚量" : "鼎广" }}
            </p>
            <p v-else>-</p>
          </FormItem> -->
          <!-- <FormItem label="合同有效期" class="appForm">
            <p v-if="ruleForm.cooperation_since && ruleForm.cooperation_until">
              {{ ruleForm.cooperation_since }} ~
              {{ ruleForm.cooperation_until }}
            </p>
            <p v-else>-</p>
          </FormItem> -->
          ...
          <FormItem class="appForm" label="开户行所在地">
            <p class="text">
              <span
                v-for="(value, index) in provinceList"
                :key="'bankProvince' + index"
                v-if="value.id === financeDate.bank_province_id"
                >{{ value.name }}</span
              >
              <span
                v-for="(value, index) in cityList2"
                :key="'bankCity' + index"
                v-if="value.id === financeDate.bank_city_id"
                >{{ value.name }}</span
              >
            </p>
          </FormItem>
          ...
          <!-- <FormItem class="appForm" label="发票类型">
            <p v-if="ruleForm.invoice_type">
              {{
                ruleForm.invoice_type === 1
                  ? "增值税普通发票"
                  : ruleForm.invoice_type === 2
                  ? "增值税专用发票（3%）"
                  : "增值税专用发票（6%）"
              }}
            </p>
          ...
          <!-- <FormItem class="appForm" label="发票项目">
            <p v-if="ruleForm.invoice_project">
              {{
                ruleForm.invoice_project === 1
                  ? "广告发布费"
                  : "非广告发布费的其他项目"
              }}
            </p>
          ...
          <!-- <FormItem class="appForm" label="补扣额外税税率">
            <p
              v-if="ruleForm.other_tax_rate && ruleForm.is_cut_other_tax === 1"
            >
              {{ getNumberFloat(ruleForm.other_tax_rate) }}%
            </p>
            <p v-else-if="ruleForm.is_cut_other_tax === 2">0%</p>
            <p v-else>-</p>
          </FormItem> -->
          <!-- <FormItem class="appForm" label="结算周期">
            <p v-if="ruleForm.settlement_period">
              {{
                ruleForm.settlement_period === 1
                  ? "预付"
                  : ruleForm.settlement_period === 2
                  ? "次月月底"
                  : ruleForm.settlement_period === 3
                  ? "次次25日前"
                  : "次次月底"
              }}
            </p>
